...
def simple_chat(prompt, model, use_stream=False):
    ...
    # print(resp["body"])
    return resp["body"]["result"].replace("```json", "").replace("```", "")
...
安徽省人民政府国有资产监督管理委员会原副主任杨东坡涉嫌严重违纪违法，目前正在接受安徽省纪委监委纪律审查和监察调查。
    ...
    prompt = """
    英特尔13、14代酷睿处运行不稳定问题愈演愈烈，服务器同样存在
    前几个月，小编向大家分享了很多有关英特尔13和14代酷睿处理器运行不稳定的问题。
看到小编又提这个话题，可能有一部分朋友会猜测：是不是这个事情现在又有了什么最新的进展，英特尔方面已经找到了确切原因，推出了可靠的解决方案，小编要分享？
——答案是否定的。目前英特尔是联合主板厂商共同推出了一些改善措施，比如发布了特殊优化版的主板BIOS固件和官方建议设置教程等等。
但是，这个问题并未从根本上解决，本质上是依靠限制、压低性能，从而间接地绕开这个问题，该方案会对处理器性能造成一定的损失，是一种治标不治本的做法。
那么，小编写这篇文章的主题究竟是什么呢？答案是分享这方面的坏消息，实际情况是上述问题现在不仅没有解决，反而愈演愈烈。
在这之前，英特尔13和14代酷睿处理器运行不稳定的问题主要表现在普通用户玩游戏的时候，会出现诸如报显存不足、游戏直接闪退，甚至死机、重启的现象，然而现在这个问题又“扩大了”。
近日科技媒体Level1Techs（图二）爆料，在服务器和数据中心上这个问题也存在。在使用W680主板和Core i9-13900K或者Core i9-14900K的游戏服务器也会出现崩溃，而且问题比较普遍。
需要强调指出的是，并不是个别类似配置的服务器存在这个问题，该机构统计了数千台类似配置的服务器，有足够多的样本，发现大约有50%的服务器都存在稳定性问题，比较普遍。
而这些服务器采用的并非是普通的消费级Z790系列主板，而是华硕和超微（Supermicro，美国企业级IT硬件厂商）这尖大品牌的高端服务器级LGA1700主板。
一般来说，企业级主板的用料和做工都非常扎实，企业用户也不会对其进行超频，基本上可以排除主板硬件层面存在缺陷的原因，极有可能是13和14代酷睿处理器自身存在问题，比如架构问题、或者英特尔在出厂时过度超频等等。
Level1Techs的统计数据表明，与搭载AMD处理器的服务器相比，搭载英特尔处理器的服务器在错误日志中出现的次数高得多。其中70%的报错来自英特尔平台，AMD平台只占约30%，请参阅上图。
现阶段，英特尔和主板厂商已经推出了部分临时性的应对方案，但严格来说，并不是“解决”，而是“改善”和“缓解”，并不能完全解决，杜绝这种问题。
对普通消费级用户来说，情况和效果可能还稍微好一点，对于企业级用户来说，服务器一般长期处于高负荷运行状态，这个问题更加严峻。
到目前为止，英特尔方面仍未对13和14代酷睿处理器运行不稳定的问题发布正式的官方说明，推出最终的解决方案，一直悬而未决。这些问题对英特尔公司的信誉造成了沉重的打击，目前已经有企业用户开始将游戏服务器更换为AMD平台。
小编会在第一时间分享相关最新动态和爆料，敬请关注。
    
    """

    comment = """
    芯片为啥没有召回制！既然有缺陷那就应该把赚的钱都吐出来！像汽车召回一样[捂脸][捂脸][捂脸][捂脸][捂脸] | 13
AMD都是大核，最新一代功耗还降下来了 | 6
花巨资买旗舰CPU，最后被降频使用！拿客户当瓜了。 | 5
笔记本平台呢？有这问题吗 | 4
英特尔技术打不到，没有台积电的5纳米工艺，就直接超频卖cpu,结果超猛了。[捂脸] | 4
英特尔目前功耗这么高，这样也有人整天鼓吹多稳定，多好，也不知道咋么想的。按说返点高，库存也差不多卖没了吧，这都好几年了，还睁眼说瞎话就不对了。 | 4
大小核的调度问题，就跟联发科以前的一核有难，9核围观一样，想稳定还是12代吧 | 3
13代，14代故障率高，厂家故意设置不保修条例，为什么没有人喷 | 3
这跟u没关系，主要是接口问题，这不新的接口马上就到 | 3
回归当年奔腾4 低效高热 被AMD 64位速龙架构按在地下摩擦 | 3
牙膏挤的多了，不思进取把自己挤成真的牙膏了 | 3
牙膏厂不会重蹈奔4的覆辙吧！那可就真要破产了哦[捂脸][捂脸][捂脸][捂脸] | 3
我认为是英特尔雇佣的印度人太多了 | 2
13600kf用了快一年了，超级稳定。 | 1
我的迷你主机是酷睿U5-125h的，看抖音也老是突然闪退，不知道是什么原因。 | 1
哦豁，估计快性能瓶颈了[偷笑] | 1
信仰崩塌了[狗头][狗头][狗头] | 1
好好的台式U搞什么异构大小核，台式机，服务器怕费电啊？他们要的是实打实的性能，不是跑分的能力，直接承认干不过AMD就完了，就整些虚头巴脑的，一点也不real。 | 1
这是一个硬件问题，是设计问题，但是美国雇佣的阿三总是用更新软件降频解决。 | 1
带k的出问题概率很大吧？ | 0
